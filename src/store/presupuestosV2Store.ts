...
/* =========================================================
   ...
   - Presupuesto principal / carátula
...
type PresupuestosV2State = {
  ...
  setFilter: <K extends keyof PresupuestosV2Filters>(key: K, value: PresupuestosV2Filters[K]) => void;
...
function calculateNoches(fechaSalida?: string | null, fechaRegreso?: string | null): number | null {
...
function buildPasajerosDetalle(adultos: number, menores: number, edadesMenores?: string | null): string {
...
function buildPresupuestoUpdatePayload(draft: UpdatePresupuestoV2Draft, currentUserId: string | null): Record<string, unknown> {
  ...
  return cleanUndefined({
    ...
    observaciones_internas: draft.observaciones_internas === "" ? null : draft.observaciones_internas,

    intro_comercial: draft.intro_comercial === "" ? null : draft.intro_comercial,

    condiciones_generales: draft.condiciones_generales === "" ? null : draft.condiciones_generales,
...
export const usePresupuestosV2Store = create<PresupuestosV2State>((set, get) => ({
  ...
  loadPresupuestoFull: async (presupuestoId, silent = false) => {
    ...
    set({
      ...
      presupuesto: presupuestoRes.data ? normalizePresupuesto(presupuestoRes.data as Record<string, unknown>) : null,
      ...
      combinaciones: ((combinacionesRes.data || []) as Record<string, unknown>[]).map(normalizeCombinacion),

      combinacionItems: ((itemsRes.data || []) as Record<string, unknown>[]).map(normalizeCombinacionItem),
  ...
  updatePresupuesto: async (draft) => {
    ...
    await Promise.all([
      ...
      get().loadPresupuestoFull(draft.id, true),

      get().loadPresupuestos(true)
  ...
  updateVuelo: async (draft) => {
    ...
    const presupuestoId = get().vuelos.find((item) => item.id === draft.id)?.presupuesto_id || get().selectedPresupuestoId;
  ...
  deleteVuelo: async (vueloId) => {
    ...
    const presupuestoId = get().vuelos.find((item) => item.id === vueloId)?.presupuesto_id || get().selectedPresupuestoId;
  ...
  updateHotel: async (draft) => {
    ...
    const payload = cleanUndefined({
      ...
      politica_cancelacion: draft.politica_cancelacion === "" ? null : draft.politica_cancelacion,
    ...
    const presupuestoId = get().hoteles.find((item) => item.id === draft.id)?.presupuesto_id || get().selectedPresupuestoId;
  ...
  deleteHotel: async (hotelId) => {
    ...
    const presupuestoId = get().hoteles.find((item) => item.id === hotelId)?.presupuesto_id || get().selectedPresupuestoId;
  ...
  updateServicio: async (draft) => {
    ...
    const presupuestoId = get().servicios.find((item) => item.id === draft.id)?.presupuesto_id || get().selectedPresupuestoId;
  ...
  deleteServicio: async (servicioId) => {
    ...
    const presupuestoId = get().servicios.find((item) => item.id === servicioId)?.presupuesto_id || get().selectedPresupuestoId;
  ...
  addCombinacion: async (draft) => {
    ...
    const payload = cleanUndefined({
      ...
      precio_total: draft.precio_total ?? 0,
  ...
  updateCombinacion: async (draft) => {
    ...
    const combinacion = get().combinaciones.find((item) => item.id === draft.id);
  ...
  addCombinacionItem: async (draft) => {
    ...
    const payload = cleanUndefined({
      ...
      orden: draft.orden || get().combinacionItems.filter((item) => item.combinacion_id === draft.combinacion_id).length + 1,
  ...
  updateCombinacionItem: async (draft) => {
    ...
    const item = get().combinacionItems.find((current) => current.id === draft.id);
  ...
  selectPresupuesto: (presupuestoId) => {
    ...
    set({ selectedPresupuestoId: presupuestoId });
  ...
  getFilteredPresupuestos: () => {
    ...
    return presupuestos.filter((presupuesto) => {
      ...
      if (filters.vendedorId !== "todos" && presupuesto.vendedor_id !== filters.vendedorId) return false;

      if (filters.sucursalId !== "todas" && presupuesto.sucursal_id !== filters.sucursalId) return false;
