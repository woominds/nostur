...
export const useCajaStore = create<CajaState>((set, get) => ({
  ...
  loadCaja: async () => {
    ...
    if (filters.moneda !== "todos") movimientosQuery = movimientosQuery.eq("moneda", filters.moneda);
    if (filters.cajaId !== "todos") movimientosQuery = movimientosQuery.eq("caja_id", filters.cajaId);
    if (filters.tipo !== "todos") movimientosQuery = movimientosQuery.eq("tipo", filters.tipo);
    if (filters.anulados === "no") movimientosQuery = movimientosQuery.eq("anulado", false);
    if (filters.anulados === "si") movimientosQuery = movimientosQuery.eq("anulado", true);
    ...
    const firstError = cajasRes.error || saldosRes.error || consolidadosRes.error || movimientosRes.error;
    ...
    set({
      ...
      movimientos: ((movimientosRes.data || []) as Partial<CajaMovimiento>[]).map(normalizeMovimiento)
    });
  },
  ...
  getMetrics: () => {
    ...
    return {
      ...
      ingresosArs: movimientos
        .filter((item) => item.moneda === "ARS" && parseMoney(item.importe_con_signo) > 0 && !item.anulado)
      ...
      ingresosUsd: movimientos
        .filter((item) => item.moneda === "USD" && parseMoney(item.importe_con_signo) > 0 && !item.anulado)
        .reduce((total, item) => total + parseMoney(item.importe_con_signo), 0),
